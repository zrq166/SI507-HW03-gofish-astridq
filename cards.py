...
class TestCard(unittest.TestCase):
	...
	def test_Deck_num(self):
		deck=Deck()
		self.assertEqual(len(deck.cards),52)

	def test_pop_card1(self):
		deck=Deck()
		onecard=deck.cards[5]
		self.assertEqual(deck.pop_card(5),onecard)

	def test_pop_card2(self):
		deck=Deck()
		cardnum=len(deck.cards)
		deck.pop_card()
		cardnum_after=len(deck.cards)
		self.assertEqual(cardnum_after,cardnum-1)

	def test_replace_card1(self):
		deck=Deck()
		deck.pop_card(0)
		cardnum1=len(deck.cards)
		deck.replace_card("Ace of Diamonds")
		cardnum2=len(deck.cards)
		self.assertEqual(cardnum1+1,cardnum2)

	def test_replace_card2(self):
		deck=Deck()
		cardnum1=len(deck.cards)
		deck.replace_card("Queen of Hearts")
		cardnum2=len(deck.cards)
		self.assertEqual(cardnum1,cardnum2)


	def test_play_war_game(self):
		tuple1 = play_war_game(True)
		self.assertTrue(isinstance(tuple1[0],str)&(len(tuple1)==3))

	#additional test.
	#Test that if you invoke the sort_cards method, the deck size is not affected
	def test_sort_cards(self):
		deck=Deck()
		len1=len(deck.cards)
		deck.sort_cards()
		self.assertEqual(len1,len(deck.cards))
